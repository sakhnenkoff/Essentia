...
final class RevenueCatPurchaseService: PurchaseService {
    ...
    func getProducts(productIds: [String]) async throws -> [AnyProduct] {
        let products: [StoreProduct] = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[StoreProduct], Error>) in
    ...
    func purchase(productId: String) async throws -> [PurchasedEntitlement] {
        let storeProducts = try await fetchStoreProducts(productIds: [productId])
    ...
    private func fetchStoreProducts(productIds: [String]) async throws -> [StoreProduct] {
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[StoreProduct], Error>) in
            Purchases.shared.getProducts(productIds) { products in
                continuation.resume(returning: products)
            }
        }
    ...
    private func purchase(storeProduct: StoreProduct) async throws -> CustomerInfo {
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<CustomerInfo, Error>) in
            Purchases.shared.purchase(product: storeProduct) { _, info, error, userCancelled in
                if let error {
                    continuation.resume(throwing: error)
                    return
                }
